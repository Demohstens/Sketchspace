...
class DragNotifier extends ChangeNotifier {
  Offset _position = Offset.zero;
  Offset get position => _position;
  bool canDrag = false;

  void initiateDrag(Offset position) {
    _position = position;
    notifyListeners();
  }

  void dragEligibilityCallback() {
    initiateDrag(position);
    notifyListeners();
  }

  void setDragibility(bool ableToDrag) {
    canDrag = ableToDrag;
    if (canDrag) {
      initiateDrag(position);
    }
...
/// Signature for when a [ManualDraggable] is dragged across the screen.
///
/// Used by [ManualDraggable.onDragUpdate].
typedef DragUpdateCallback = void Function(DragUpdateDetails details);

/// Signature for when a [ManualDraggable] is dropped without being accepted by a [DragTarget].
///
/// Used by [ManualDraggable.onDraggableCanceled].
...
///
/// Used by [ManualDraggable.onDragEnd].
typedef DragEndCallback = void Function(DraggableDetails details);

/// Signature for when a [ManualDraggable] leaves a [DragTarget].
///
/// Used by [DragTarget.onLeave].
typedef DragTargetLeave<T> = void Function(T? data);

/// Signature for when a [ManualDraggable] moves within a [DragTarget].
///
/// Used by [DragTarget.onMove].
typedef DragTargetMove<T> = void Function(DragTargetDetails<T> details);

/// Signature for the strategy that determines the drag start point of a [ManualDraggable].
///
/// Used by [ManualDraggable.dragAnchorStrategy].
///
...
typedef DragAnchorStrategy = Offset Function(
    ManualDraggable<Object> draggable, BuildContext context, Offset position);
...
///  * [ManualDraggable.dragAnchorStrategy], for which this is a built-in value.
...
///
/// {@tool dartpad}
/// The following example has a [ManualDraggable] widget along with a [DragTarget]
...
class ManualDraggable<T extends Object> extends StatefulWidget {
  ...
  /// event, see [ManualDraggable.affinity].
  final Axis? axis;
  ...
  /// at the location of the [ManualDraggable] itself when a drag is under way.
  ...
  /// starts, see [ManualDraggable.axis].
  final Axis? affinity;
  ...
  /// The notifier used to simbolize a drag event.
  final DragNotifier dragNotifier;
  ...
  @override
  State<ManualDraggable<T>> createState() => _DraggableState<T>(dragNotifier);
}

class _DraggableState<T extends Object> extends State<ManualDraggable<T>> {
  _DraggableState(this._dragNotifier);

  final DragNotifier _dragNotifier;
  @override
  void initState() {
    super.initState();
    _dragNotifier.setDragibility(true);
    _recognizer = widget.createRecognizer(_startDrag);
    _dragNotifier.addListener(dragStateUpdater(_dragNotifier.position));
  }
  ...
  // case of removing the ManualDraggable from the tree in the middle of a drag. That
  ...
  VoidCallback dragStateUpdater(pos) {
    _startDrag(pos);
    return () {
      print("Drag state changed");
    };
  }

  _DragAvatar<T>? _startDrag(Offset position) {
    print("drag started");
    final int? maxSimultaneousDrags = widget?.maxSimultaneousDrags;
    if (maxSimultaneousDrags != null) {
      if (_activeCount >= maxSimultaneousDrags) {
        return null;
      }
    }
...
/// A widget that receives data when a [ManualDraggable] widget is dropped.
///
...
class DragTarget<T extends Object> extends StatefulWidget {
  ...
  /// Called when a [ManualDraggable] moves within this [DragTarget]. It will not be
