...
class Stroke extends SketchElement {
  SketchPath path;
  Paint paint;
  // Used for temporary storage during scaling
  Rect _initialBoundsOnScaleStart = Rect.zero;
  List<Offset> _initialPointsOnScaleStart = []; // Store original points
  HandlePosition? _activeHandle; // Store the active handle

  Rect get initialBoundsOnScaleStart => _initialBoundsOnScaleStart;

  Stroke({required this.path, required this.paint, required super.layerId});
  ...
  bool hitTest(Offset point) {
    ...
    final double baseThreshold =
        10.0; // Base threshold for better touch/click detection
    final double tolerance = max(
      strokeWidth * 1.5,
      baseThreshold,
    ); // Use larger of stroke-based or minimum threshold
    ...
    if (points.length < 2) {
      ...
      return points.isNotEmpty &&
          (points.first - point).distanceSquared < toleranceSq;
    }
  ...
  @override
  void startScaling(HandlePosition handle) {
    _initialPointsOnScaleStart = List.from(path.originalPoints);
    _initialBoundsOnScaleStart = boundary; // Capture initial bounds
    _activeHandle = handle; // Store the active handle
  }
  ...
  scale(Vector2 delta, Offset pivot) {
    path.scale(delta.x, delta.y, pivot, _initialPointsOnScaleStart);
  }
      ...
      paint:
          Paint()
            ...
            ..style = PaintingStyle.values.firstWhere(
              (style) =>
                  style.toString().split('.').last == json["paint"]["style"],
            ),
...
double _pointSegmentDistanceSq(Offset C, Offset A, Offset B) {
  ...
  // t = [(C - A) . (B - A)] / |B - A|^2
  final double t =
      ((C.dx - A.dx) * (B.dx - A.dx) + (C.dy - A.dy) * (B.dy - A.dy)) / l2;
