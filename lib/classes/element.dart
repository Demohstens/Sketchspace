...
class Stroke extends DrawingElement {
  ...
  @override
  Rect get boundary => path.path.getBounds();
  ...
  final double tolerance = (strokeWidth / 2.0) + 1.5; // Tolerance radius
  final double toleranceSq = tolerance * tolerance; // Use squared distance

  // Need access to the original points that define the segments
  final List<Offset> points = path.points; // Assuming SketchPath has this
  ...
  for (int i = 0; i < points.length - 1; i++) {
    ...
    if (distSq < toleranceSq) {
      // print("Hit segment $i, DistSq: $distSq, TolSq: $toleranceSq");
      return true; // Hit detected!
    }
  }

  // If no segment was hit
  return false;
}// end hitTest
